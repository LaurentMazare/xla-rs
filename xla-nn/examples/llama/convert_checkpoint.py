...
# Linear weights are kept in the PyTorch [out_features, in_features] layout,
# which is what xla_nn::Linear expects.
def convert_state_dict(state_dict: Dict[str, torch.Tensor], dtype: torch.dtype = torch.float16) -> Dict[str, torch.Tensor]:
    ...
    for layer_idx in sorted(set([k.split(".")[1] for k in state_dict if k.startswith("layers")])):
        ...
        converted[f"transformer.h.{layer_idx}.attn.c_attn.weight"] = np.concatenate(
            (
...
def convert_weights(llama_ckpt, *, output_path: Path = Path("llama.safetensors"), dtype: str = "float16") -> None:
