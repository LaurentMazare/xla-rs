...
impl XlaBuilder {
    ...
    /// A custom call to a backend-specific target: `operands` are the call's
    /// tensor inputs, the result has element type `ty` and shape `dims`, and
    /// `opaque` is the backend config passed through verbatim. On GPU the
    /// target is a function registered with
    /// [`register_custom_call_target`](crate::register_custom_call_target).
