...
impl In {
    ...
    fn upload(&self, client: &PjRtClient) -> Result<PjRtBuffer> {
        ...
        match self {
            ...
            In::Bf16(_, v) => client.buffer_from_host_raw_bytes(ElementType::Bf16, &bf16_bytes(v), &dims, None),
...
fn compare(name: &str, a: &[f32], b: &[f32]) {
    ...
    let verdict = if nan_b > nan_a || rel > 2e-2 || rel.is_nan() { "MISMATCH" } else if rel > 2e-3 { "loose" } else { "ok" };
    eprintln!(
...
fn plugin_parity() -> Result<()> {
    ...
    cases.push((
        "linear_bf16",
        vec![In::Bf16(vec![32, d], rand(1, (32 * d) as usize, 1.0)), In::Bf16(vec![d, 1024], rand(2, (d * 1024) as usize, 0.02))],
    ...
    cases.push((
        ...
        Box::new(move |bl, p| {
            ...
            let mask = valid.select(&bl.c0(0f32)?.broadcast(&[b, 1, c])?, &bl.c0(f32::NEG_INFINITY)?.broadcast(&[b, 1, c])?)?;
            let mask = mask.convert(PrimitiveType::Bf16)?.broadcast_in_dim(&[b, h, 1, c], &[0, 2, 3])?;
            let probs = attn.add_(&mask)?.convert(PrimitiveType::F32)?.softmax(-1)?.convert(PrimitiveType::Bf16)?;
    ...
    cases.push((
        ...
        Box::new(move |bl, p| {
            let scale = bl.c0(1f32 / (hd as f32).sqrt())?.convert(PrimitiveType::Bf16)?;
            let attn = p[0].dot_general(&p[1], &[3], &[3], &[0, 1], &[0, 2])?.mul_(&scale)?.convert(PrimitiveType::F32)?;
            let jj = bl.iota(ElementType::S32, &[b, 1, c], 2)?;
            let valid = jj.le(&bl.c0(150i32)?.broadcast(&[b, 1, c])?)?;
            let mask = valid.select(&bl.c0(0f32)?.broadcast(&[b, 1, c])?, &bl.c0(f32::NEG_INFINITY)?.broadcast(&[b, 1, c])?)?;
            let probs = attn.add_(&mask.broadcast_in_dim(&[b, h, 1, c], &[0, 2, 3])?)?.softmax(-1)?.convert(PrimitiveType::Bf16)?;
    ...
    cases.push(("gelu_erf_f32", vec![In::F32(vec![16, 4096], rand(7, 16 * 4096, 3.0))], Box::new(|_, p| p[0].gelu_erf())));
    cases.push(("gelu_erf_bf16_roundtrip", vec![In::Bf16(vec![16, 4096], rand(8, 16 * 4096, 3.0))], Box::new(|_, p| p[0].convert(PrimitiveType::F32)?.gelu_erf()?.convert(PrimitiveType::Bf16))));
    cases.push((
        "onehot_lookup_bf16",
        vec![In::Bf16(vec![2049, 512], rand(9, 2049 * 512, 1.0)), In::S32(vec![32], (0..32).map(|i| if i % 7 == 3 { -1 } else { (i * 61) % 2049 }).collect())],
        Box::new(|bl, p| {
            let iota = bl.iota(ElementType::S32, &[32, 2049], 1)?;
            let onehot = iota.eq(&p[1].broadcast_in_dim(&[32, 2049], &[0])?)?.convert(PrimitiveType::Bf16)?;
            onehot.dot_general(&p[0], &[1], &[0], &[], &[])
        }),
    ));
    cases.push((
        "gather_lookup_bf16",
        vec![In::Bf16(vec![2049, 512], rand(9, 2049 * 512, 1.0)), In::S32(vec![32], (0..32).map(|i| (i * 61) % 2049).collect())],
        Box::new(|_, p| p[0].take(&p[1], 0)),
    ));
    cases.push((
        "rotary_bf16",
        vec![In::Bf16(vec![b, h, 1, hd], rand(10, (b * h * hd) as usize, 1.0)), In::F32(vec![1, 64], rand(11, 64, 1.0)), In::F32(vec![1, 64], rand(12, 64, 1.0))],
        Box::new(move |_, p| {
            let half = hd / 2;
            ...
            let cos = p[1].convert(PrimitiveType::Bf16)?.broadcast_in_dim(&[b, h, 1, half], &[2, 3])?;
            let sin = p[2].convert(PrimitiveType::Bf16)?.broadcast_in_dim(&[b, h, 1, half], &[2, 3])?;
    ...
    cases.push((
        "rmsnorm_f32_of_bf16",
        vec![In::Bf16(vec![32, 1, d], rand(13, (32 * d) as usize, 2.0)), In::Bf16(vec![d], rand(14, d as usize, 1.0))],
        Box::new(move |bl, p| {
            ...
            x.mul_(&inv.broadcast_in_dim(&[32, 1, d], &[0, 1, 2])?)?.mul_(&w)?.convert(PrimitiveType::Bf16)
        }),
    ));
    cases.push((
        "layernorm_f32",
        vec![In::F32(vec![32, 1024], rand(15, 32 * 1024, 2.0)), In::F32(vec![1024], rand(16, 1024, 1.0)), In::F32(vec![1024], rand(17, 1024, 0.1))],
        Box::new(|_, p| p[0].layer_norm(-1, &p[1].broadcast_in_dim(&[32, 1024], &[1])?, &p[2].broadcast_in_dim(&[32, 1024], &[1])?)),
    ));
    cases.push((
        ...
        Box::new(|_, p| p[0].scatter(&p[1], &p[2], &assign(ElementType::Bf16)?, &[1, 2], &[0, 1], &[0, 1], 1)),
    ));
    cases.push((
        ...
        Box::new(move |bl, p| {
            ...
            let pj = q_end.sub_(&q_end.sub_(&jj)?.rem_(&bl.c0(context as i32)?.broadcast(&dims)?)?)?;
    ...
    cases.push((
        "conv1d_f32",
        vec![In::F32(vec![2, 64, 100], rand(20, 2 * 64 * 100, 1.0)), In::F32(vec![64, 64, 7], rand(21, 64 * 64 * 7, 0.05))],
        Box::new(|_, p| p[0].conv1d(&p[1], 1, 3, 1, 1)),
    ));
    cases.push((
        "conv_transpose1d_f32",
        vec![In::F32(vec![2, 64, 50], rand(22, 2 * 64 * 50, 1.0)), In::F32(vec![64, 64, 8], rand(23, 64 * 64 * 8, 0.05))],
        Box::new(|_, p| p[0].conv_transpose1d(&p[1], 4, 0, 0, 1, 1)),
    ));
    cases.push(("rev_f32", vec![In::F32(vec![3, 5, 7], rand(24, 105, 1.0))], Box::new(|_, p| p[0].rev(&[2]))));
    cases.push((
        "elementwise_f32",
    ...
    cases.push((
        ...
        Box::new(|_, p| {
            ...
            let am = x.argmax(ElementType::S32, -1)?.reshape(&[32, 1])?.convert(PrimitiveType::F32)?;
    ...
    for (name, inputs, build) in cases.iter() {
        ...
        match run(&dev, name, inputs, build) {
            ...
            Err(e) => eprintln!("PLUGIN FAIL {name}: {}", e.to_string().lines().next().unwrap_or("")),
        }
    }
    // Uniform sampling statistics (no CPU reference: different generators).
    {
        ...
        let (_s, u) = rng.sample_uniform(xla::RandomAlgorithm::Default, ElementType::F32, &[1024, 1024], 0.0, 1.0)?;
        ...
        let (lo, hi) = (u.iter().cloned().fold(f32::MAX, f32::min), u.iter().cloned().fold(f32::MIN, f32::max));
        let distinct = { let mut s = u.iter().map(|v| v.to_bits()).collect::<Vec<_>>(); s.sort_unstable(); s.dedup(); s.len() };
